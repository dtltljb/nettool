...
void frmTcpServer::initForm()
{
    ...
    connect(tcpServer, SIGNAL(clientConnected(QString, int)), this, SLOT(clientConnected(QString, int)));       //connected
...
void frmTcpServer::on_btnSend_clicked()
{
    ...
    auto data = ui->cboxData->currentText();
    //QString data = ui->cboxData->currentText();
    if (data.length() <= 0) {
        return;
    }
 //   auto buf  = QByteArray::fromHex(data.toLocal8Bit());
    sendData(data);
}
